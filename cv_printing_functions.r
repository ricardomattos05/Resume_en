...
create_CV_object <-  function(data_location,
  ...
  if (is_google_sheets_location) {
    ...
    cv$entries_data <-
      readr::read_csv(paste0(data_location, "entries.csv"), skip = 1)
    cv$skills       <-
      readr::read_csv(paste0(data_location, "language_skills.csv"), skip = 1)
    cv$text_blocks  <-
      readr::read_csv(paste0(data_location, "text_blocks.csv"), skip = 1)
    cv$contact_info <-
      readr::read_csv(paste0(data_location, "contact_info.csv"), skip = 1)
  }
  ...
  parse_dates <- function(dates) {
    date_month <-
      stringr::str_extract(dates, "(\\w+|\\d+)(?=(\\s|\\/|-)(20|19)[0-9]{2})")
  ...
  # Clean up ntries dataframe to format we need it for printing
  cv$entries_data %<>%
    tidyr::unite(
    ...
    dplyr::arrange(desc(parse_dates(start)), desc(parse_dates(end))) %>%
...
sanitize_links <- function(cv, text) {
  if (cv$pdf_mode) {
    ...
    if (n_new_links > 0) {
      ...
      link_superscript_mappings <- purrr::set_names(paste0("<sup>", (1:n_new_links) + n_links, "</sup>"),
...
print_section <-
  function(cv, section_id, glue_template = "default") {
...
\n\n\n"
    ...
    section_data <-
      dplyr::filter(cv$entries_data, section == section_id)
...
print_contact_info <- function(cv) {
  glue::glue_data(cv$contact_info,
                  "- <i class='fa fa-{icon}'></i> {contact}") %>% print()
